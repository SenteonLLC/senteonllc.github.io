...
Senteon Users can observe and manage their fleet of Endpoints in Command Center. When Senteon Agent is installed on an Endpoint, it will register itself with Senteon Master. Once registered, it will appear in Command Center uner the relevant Managed Account
...
| Actions | Depending on the current status of the Senteon Agent, different actions can be performed. Further detail can be found [here](#modifying-endpoints) |
