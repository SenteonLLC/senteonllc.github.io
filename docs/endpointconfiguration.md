...
## Endpoints
Once Senteon Agent is installed on an Endpoint, it will register itself with the Senteon Master. Once registered it will appear in Command Center in the main `Endpoints` page of the Managed Account Console
...
| Endpoint ID | A unique identifier generated by Senteon to ensure every endpoint can be referenced even if a hostname is not unique |
| Group | The current group that the endpoint belongs to and is deriving its Active Configuration Set from |
